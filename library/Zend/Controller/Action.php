...
//        $value = mysql_real_escape_string($value);
        ...
        (new DB_IDLogger())->log(Zend_Auth::getInstance()->getIdentity()->id, array($paramName => $value));
     ...
     * @return array
     */
    protected function _getAllParams($text = false, $json = false, $notReplace = false)
    {
        $params = $this->getRequest()->getParams();

        if(!$notReplace) {
            if ($text) {
                $jsxss = "`,{,},=,+,>,<,!,#,$,%,^,&,*,;,',\\,:,--,IF,AND,OR,LIKE,SELECT,FROM,DROP,'TRUNCATE','DELETE',onabort,oncanplay,oncanplaythrough,ondurationchange,onemptied,onended,onerror,onloadeddata,onloadedmetadata,onloadstart,onpause,onplay,onplaying,onprogress,onratechange,onseeked,onseeking,onstalled,onsuspend,ontimeupdate,onvolumechange,onwaiting,oncopy,oncut,onpaste,ondrag,ondragend,ondragenter,ondragleave,ondragover,ondragstart,ondrop,onblur,onfocus,onfocusin,onfocusout,onchange,oninput,oninvalid,onreset,onsearch,onselect,onsubmit,onabort,onbeforeunload,onerror,onhashchange,onload,onpageshow,onpagehide,onresize,onscroll,onunload,onkeydown,onkeypress,onkeyup,altKey,ctrlKey,shiftKey,metaKey,keyCode,which,charCode,location,onclick,ondblclick,oncontextmenu,onmouseover,onmouseenter,onmouseout,onmouseleave,onmouseup,onmousemove,onwheel,altKey,ctrlKey,shiftKey,metaKey,button,buttons,which,clientX,clientY,detail,relatedTarget,screenX,screenY,deltaX,deltaY,deltaZ,deltaMode,animationstart,animationend,animationiteration,animationName,elapsedTime,propertyName,elapsedTime,transitionend,onerror,onmessage,onopen,ononline,onoffline,onstorage,onshow,ontoggle,onpopstate,ontouchstart,ontouchmove,ontouchend,ontouchcancel,persisted,javascript";
                $jsxss = explode(",", $jsxss);


                foreach ($params as $key => $elem) {

                    $params[$key] = preg_replace("'<script[^>]*?>.*?</script>'si", "", $params[$key]);
                    $params[$key] = str_replace($jsxss, "", $params[$key]);
                    $params[$key] = htmlentities($params[$key], ENT_QUOTES, "UTF-8");
                    $params[$key] = htmlspecialchars($params[$key], ENT_QUOTES);
